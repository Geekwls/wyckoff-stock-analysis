...
### 因果法则目标测算与区间失效 (Invalidated TR)

在 v3.1.0 中，我们重构了点数图 (P&F) 因果测算引擎，并引入了 **交易区间失效风控机制 (Invalidated TR)**：
1. **自动失效拦截**：当价格实质性跌破吸筹区间底部（或升破派发区间顶部）且未能在规定时间内拉回时，系统会将 `invalidated_tr` 标记为 `True`，并 **主动暂停垂直目标价测算**。
2. **威科夫风控哲学**：一旦原交易区间失效，旧的因果关系不复存在，盲目测算目标价极度危险。此时系统会提示您“原区间已失效，必须重建结构”。

### LPS (支撑极限点) 与 LPSY (最后供应点) 混淆？

二期重构对这两个极易混淆的概念在代码层和分析报告中进行了严格的语境隔离：
- **LPS (Last Point of Support)**：仅存在于 **吸筹 (Accumulation) / 重新吸筹** 阶段的 Phase C/D，是伴随成交量萎缩的回踩支撑点，是**多头建仓的最佳时机**。系统支持 0.5~1.5x ATR 动态波动率溢价，避免虚假跌破。
- **LPSY (Last Point of Supply)**：仅存在于 **派发 (Distribution) / 重新派发** 阶段，是主力出货完毕后，弱势反弹（低量、无法收复阻力）的极限点，是**空头入场信号**。

### SOS 向量化与迭代版结果不一致？

v3.1.0 已完成全面对齐：
1. 修复了向量化版 SOS 命名冲突 Bug，确保两套计算路径算法 100% 对齐。
2. 统一了 JOC 的回测窗口为 15 日，且迭代版完美补齐了 JOC 测试质量打分，两者输出完全一致，回测提速 10 倍以上。

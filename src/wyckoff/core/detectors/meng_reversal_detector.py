...
class MengReversalDetector(BaseDetector):
    ...
    def _detect_spring_enhanced_vectorized(self) -> Dict:
        ...
        t = self.thresholds
        valid_breakdown = (lows < support_levels) & (breakdown_pcts >= t.MENG_SPRING_BREAKDOWN_MIN) & (breakdown_pcts <= t.MENG_SPRING_BREAKDOWN_MAX)
    ...
    def _calculate_spring_confidence(self, breakdown_pct, recovery_days, vol_ratio, close_position):
        score = 0
        if 1.5 <= breakdown_pct <= 2.5: score += 25
        elif 1 <= breakdown_pct <= 3: score += 20
        if recovery_days == 2: score += 25
        elif recovery_days <= 3: score += 20
        ...
        elif close_position >= 60: score += 15
        return score

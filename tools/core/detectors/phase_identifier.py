...
class PhaseIdentifier:
    ...
    def identify(self, events: Dict) -> Dict:
        ...
        if phase == 'Unknown':
            phase, confidence = self._fallback_logic()

        ma_conf = self._check_ma_confirmation(phase)
        vol_conf = self._check_volume_confirmation(phase)
    ...
    def _determine_phase_from_events(self, events: Dict) -> Tuple[str, float]:
        ...
        if is_spring and is_sos: return 'Accumulation Phase D (积累期突破)', 0.85
        if is_spring: return 'Accumulation Phase C (积累期震仓)', 0.70
        if is_upthrust and is_sow: return 'Distribution Phase D (派发期跌破)', 0.85
        if is_upthrust: return 'Distribution Phase C (派发期诱多)', 0.70
        if cl.get('detected') and ar.get('detected'):
            if cl.get('type') == 'selling_climax': return 'Accumulation Phase A (恐慌抛售停止)', 0.75
            return 'Distribution Phase A (买入高潮停止)', 0.75
        if st.get('detected'):
            if cl.get('type') == 'selling_climax': return 'Accumulation Phase B (积累期测试)', 0.60
            return 'Distribution Phase B (派发期测试)', 0.60
        return 'Unknown', 0.30

    def _fallback_logic(self) -> Tuple[str, float]:
        ma20, ma50, ma200 = self.data['MA20'].iloc[-1], self.data['MA50'].iloc[-1], self.data['MA200'].iloc[-1]
        current = self.data['Close'].iloc[-1]
        if current > ma20 > ma50 > ma200: return "Markup Phase E (强势上涨)", 0.6
        if current < ma20 < ma50 < ma200: return "Markdown Phase E (强势下跌)", 0.6
        return "Trending (趋势中)", 0.4

    def _check_ma_confirmation(self, phase: str) -> float:
        ma200, current = self.data['MA200'].iloc[-1], self.data['Close'].iloc[-1]
        if 'Markup' in phase or 'Accumulation' in phase: return 0.8 if current > ma200 else 0.4
        if 'Markdown' in phase or 'Distribution' in phase: return 0.8 if current < ma200 else 0.4
        return 0.5
    ...
    def _check_volume_confirmation(self, phase: str) -> float:
        ...
        if 'Markup' in phase: return 0.9 if ratio > 1.2 else 0.5
        if 'Markdown' in phase: return 0.9 if ratio < 0.8 else 0.5
        return 0.5
